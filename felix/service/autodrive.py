...
from felix.types import Measurement, Twist
import torch
import torchvision
import numpy as np
...
class AutoDriveService(BaseService):
    ...
    def _on_tof(self, payload: dict):
        measurement = Measurement.model_validate(payload.get("message"))
        self.tof[str(measurement.id)] = measurement.value
        self.logger.debug(f"TOF {self.tof}")
...
class ObstacleAvoider(AutoDriveService):
...
async def run(frequency: int = 10):
    ...
    await autodrive.spin(1/frequency)


@click.command()
@click.option('--frequency', default=20, help='Frequency in Hz')
