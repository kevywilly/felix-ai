...
from lib.interfaces import Twist, Odometry
from lib.kinematics import Kinematics
from felix.vision.image import Image, ImageUtils
...
class Robot(BaseNode):
    ...
    def _setup_subscriptions(self):
        if self._camera is not None:
            ...
            if self._controller is not None:
                traitlets.dlink(
                    (self._camera, "value"), (self._controller, "camera_image")
                )
        if self._controller is not None:
    ...
    def handle_navigate(self, data: Dict) -> str:
        ...
        if captureMode:
            captured_image = self._image_collector.save_navigation_image(
                x, y, w, h, self.get_image()
            )
    ...
    def _save_nav_data_to_csv(self, image_filepath: str, nav_data: np.ndarray):
        ...
        csv = image_filepath + "," + ",".join([f"{item}" for item in nav_data])
