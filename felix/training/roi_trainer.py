...
class ROIObstacleTrainer(Trainer):
    ...
    def __init__(
        ...
        roi_type='ground',
        roi_height_ratio=settings.roi_height_ratio,
        roi_width_ratio=1.0,
        ...
        *args, **kwargs
        ...
        logger.info(
            ...
            f"\tROI Configuration:\n"
            f"\t\troi_type: {self.roi_type}\n"
            f"\t\troi_height_ratio: {self.roi_height_ratio}\n"
            f"\t\troi_width_ratio: {self.roi_width_ratio}\n"
            f"\t\troi_vertical_offset: {self.roi_vertical_offset}\n"
        )

    def _get_roi_transform(self):
        if self.roi_type == 'none':
            return None
        else:
            return ROITransform(
    ...
    def _get_dataset(self):
        ...
        transform_list.extend([
            ...
            transforms.Resize((224, 224)),  # Resize (potentially cropped) image to 224x224
            transforms.ToTensor(),
    ...
    def _get_model(self):
        ...
        """
        if self.model_type == ModelType.resnet_50:
            model = resnet50(weights=ResNet50_Weights.DEFAULT)
            num_ftrs = model.fc.in_features
            model.fc = torch.nn.Linear(num_ftrs, self.num_categories)
            
        elif self.model_type == ModelType.mobilenet_large:
            model = mobilenet_v3_large(weights=MobileNet_V3_Large_Weights.DEFAULT)
            # MobileNetV3 classifier is a Sequential with Linear layer at index 3
            model.classifier[3] = torch.nn.Linear(model.classifier[3].in_features, self.num_categories)
            
        elif self.model_type == ModelType.mobilenet_small:
            model = mobilenet_v3_small(weights=MobileNet_V3_Small_Weights.DEFAULT)
            # MobileNetV3 classifier is a Sequential with Linear layer at index 3
            model.classifier[3] = torch.nn.Linear(model.classifier[3].in_features, self.num_categories)
            
        elif self.model_type == ModelType.alexnet:
    ...
    def train(self):
        ...
        for epoch in range(self.epochs):
            ...
            logger.info(f"Epoch {epoch}: Accuracy={test_accuracy:.4f}, Loss={avg_loss:.4f}")
            ...
            if best_accuracy >= self.early_stop_threshold:
                logger.info(f"Early stopping as accuracy {best_accuracy:.4f} reached threshold {self.early_stop_threshold}")
                break
    ...
    def visualize_roi_samples(self, num_samples=5, save_path=None):
        ...
        try:
            ...
            fig.suptitle(f'ROI Visualization - {self.roi_type} ROI', fontsize=16)
            ...
            for i in range(min(num_samples, len(dataset))):
                ...
                transformed_img = transformed_tensor * std[:, None, None] + mean[:, None, None]
...
def create_ground_robot_trainer(images_path, model_file, model_type=settings.model_type, **kwargs):
    """
    Factory function for creating a trainer optimized for ground robots
    """
    return ROIObstacleTrainer(
        images_path=images_path,
        model_file=model_file,
        model_type=model_type,                              # Default to MobileNet for indoor robots
        roi_type='ground',
        roi_height_ratio=settings.roi_height_ratio,         # Focus on bottom 60%
        roi_vertical_offset=settings.roi_vertical_offset,   # Start 40% down from top
        roi_width_ratio=1.0,                                # Use full width
        **kwargs
    )
...
if __name__ == "__main__":
    # Example usage
    trainer = create_ground_robot_trainer(
        images_path="/path/to/obstacle/images",
    ...
    # Visualize what ROI is doing
    trainer.visualize_roi_samples(num_samples=3)
    
    # Train the model
    best_accuracy = trainer.train()
    print(f"Training completed with best accuracy: {best_accuracy}")