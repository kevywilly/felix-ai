...
class MockController(Node):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._bot = MockRosmaster(car_type=2, com=self.vehicle.yaboom_port)
    ...
    def spinner(self):
        ...
        if (
            self.autodrive
            and self.autodriver is not None
    ...
    def _scale_cmd_vel(self, cmd: Twist):
        return (
            ...
            if cmd.linear.x > self.vehicle.max_linear_velocity
            else cmd.linear.x,
            self.vehicle.max_linear_velocity
            if cmd.linear.y > self.vehicle.max_linear_velocity
            else cmd.linear.y,
            self.vehicle.max_angular_velocity
            if cmd.angular.z > self.vehicle.max_angular_velocity
            else cmd.angular.z,
        )
    ...
    def _apply_cmd_vel(self, cmd: Twist):
        ...
        vel = self.vehicle.forward_kinematics(vx, vy, omega)
    ...
    def _apply_nav_target(self):
        if self.nav_target:
            self._apply_cmd_vel(self.nav_target.twist)
            self._start_nav()
        else:
            self.stop()
